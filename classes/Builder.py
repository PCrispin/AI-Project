...
class Builder:
    ...
    def create(self, xCenter: int, zCenter: int, orientation :orientations, building: building, requiredWidth: int = -1, requiredDepth: int = -1) -> bool :
        ...
        def findHeight(x: int, z: int) -> Tuple[int, int]:
            ...
            if 0 <= wX < world_slice.rect[2] and 0 <= wZ < world_slice.rect[3] :
                return world_slice.heightmaps['MOTION_BLOCKING_NO_LEAVES'][wX][wZ], world_slice.heightmaps['MOTION_BLOCKING'][wX][wZ]
            else:
                ...
                for z in range(min(z1, z2), max(z1, z2) + 1) :
                    ...
                    if yFloor != yObjects :
                        for yLumberjack in range(yFloor, yObjects + 1):
                            placeBlockBatched(x, yLumberjack, z, block_codes.AIR.value, BLOCK_BATCH_SIZE)
        ...
        def buildFoundationsInRegion(x1 :int, z1: int, x2: int, z2 :int, floorY):
            ...
            for x in range(smallX, bigX + 1) :
                if x - smallX % 20 == 0 :
                    print("Foundations {:.0f}% built.".format(100*(x - smallX)/(bigX - smallX)))
                for z in range(min(z1, z2), max(z1, z2) + 1) :
                    for y in range(findHeight(x, z)[0], floorY + 1) :
                        placeBlockBatched(x, y, z, block_codes.BEDROCK.value, BLOCK_BATCH_SIZE)
            sendBlocks()
        ...
        yMin, yOffset, isOverWater = findHeightInRegion(site.coords[0], site.coords[1], site.coords[2], site.coords[3])
            ...
            for block in buildingReader:
                ...
                for xRepeat in range(1, xRepeatCount[x] + 1) :
                    ...
                    for zRepeat in range(1, zRepeatCount[z] + 1) :
                        buildBlockRotated(blockType, xShifted + xRepeat, y, zShifted + zRepeat)
